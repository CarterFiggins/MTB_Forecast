...
function calculateWind(wind: WindInterface): cssWarning {
...
export default function WeatherTable({ list }: List) {
  ...
  const dayData = showNight
    ? list
    : _.filter(list, (data) => data.sys.pod === "d");
  // const dayData = list
  return (
    <div>
      <button onClick={() => setShowNight((c) => !c)}>
        {showNight ? "Hide Nights" : "Show Nights"}
      </button>

      <div className="weather-list">
        {_.map(dayData, (data: ReportInfoInterface) => {
